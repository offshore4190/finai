#!/usr/bin/env python3
"""
NYSE Full Backfill - Discovers and downloads all filings for NYSE companies.
This script processes all ~2,353 NYSE companies for 2023-2025 filings.
"""
import sys
...
def discover_filings():
    """Discover all filings for NYSE companies (2023-2025)."""
    logger.info("nyse_backfill_started", phase="discovery")

    sec_client = SECAPIClient()

    with get_db_session() as session:
        # Get all NYSE companies
        companies = session.query(Company).filter(
            Company.exchange == 'NYSE'
        ).order_by(Company.ticker).all()

        total_companies = len(companies)
        logger.info("nyse_companies_loaded", count=total_companies)
        ...
        # Final summary
        logger.info(
            "nyse_backfill_discovery_completed",
            companies_processed=companies_processed,
...
def download_artifacts():
    """
    Download all pending artifacts for NYSE companies with concurrent workers.
    ...
    logger.info("nyse_artifacts_found", total=total, workers=settings.download_workers)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='NYSE Full Backfill - Discover and download all filings'
    )
    parser.add_argument(
        '--discover-only',
        action='store_true',
    ...
    try:
        ...
        logger.info(
            "nyse_backfill_completed",
            duration_seconds=duration,
            duration_hours=f"{duration/3600:.2f}"
        )

    except KeyboardInterrupt:
        logger.info("nyse_backfill_interrupted")
        sys.exit(1)
    except Exception as e:
        logger.error("nyse_backfill_failed", error=str(e), exc_info=True)
        sys.exit(1)
